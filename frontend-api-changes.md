...
## Breaking Changes - Historical NAV Endpoint
...
```bash
# Group by holding company (NEW)
GET /portfolio/aggregations/historical?group_by=holding_company

# Group by ownership entity (old behavior)
...
**Example response with `group_by=holding_company`:**
```json
{
  "series": [
    {
      "name": "Isis Invest",
      "data": [
        {"date": "2025-05-01", "value_usd": 48240000, "value_eur": 44500000},
        {"date": "2025-06-01", "value_usd": 49100000, "value_eur": 45200000}
      ]
    },
    {
      "name": "Other Holding Co",
      "data": [...]
    }
  ]
}
...
- [ ] Update `/aggregations/historical` to use `group_by` instead of `group_by_entity`
...
- [ ] Update Historical NAV chart to use `group_by` parameter instead of `group_by_entity`
...
- [ ] Verify Historical NAV chart works with `group_by=holding_company`
...
- `GET /portfolio/aggregations/historical` - **`group_by_entity` → `group_by`** + query params renamed
